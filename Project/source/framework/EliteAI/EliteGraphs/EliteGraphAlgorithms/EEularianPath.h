...
namespace Elite
{
	...
	inline Eulerianity EulerianPath<T_NodeType, T_ConnectionType>::IsEulerian() const
	{
		...
		for (auto n : nodes)
		{
			...
			if (connections.size() & 1)	// bitwise AND 
				oddCount++;
		}
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		int currentIdx = invalid_node_index;
		auto nodes = m_pGraph->GetAllNodes();
		...
		for (auto n : nodes)
		{
			...
			{
				currentIdx = n->GetIndex();
				break;
			}
		}

		if (currentIdx == invalid_node_index)
			currentIdx = nodes[0]->GetIndex();

		// Start algorithm loop
		std::stack<int> nodeStack;

		while (graphCopy->GetNodeConnections(currentIdx).size() > 0 || nodeStack.size() > 0)
		{
			if (graphCopy->GetNodeConnections(currentIdx).size() > 0)
			{
				nodeStack.push(currentIdx);
				auto connection = graphCopy->GetNodeConnections(currentIdx).front();
				currentIdx = connection->GetTo();
				graphCopy->RemoveConnection(connection);
			}
			else
			{
				path.push_back(m_pGraph->GetNode(currentIdx));
				currentIdx = nodeStack.top();
				nodeStack.pop();
			}
		}
		path.push_back(m_pGraph->GetNode(currentIdx));
