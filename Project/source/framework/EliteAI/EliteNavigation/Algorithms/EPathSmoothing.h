	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				if (rightLeg.Cross(newRightLeg) >= 0.f)	// moving inwards
				{
					//2. See if new line degenerates a line segment - RIGHT
					if (newRightLeg.Cross(leftLeg) < 0.f)		// legs crossed
					{
						apexPos += leftLeg;
						apexIdx = leftLegIdx;
				...
				if (leftLeg.Cross(newLeftLeg) <= 0.f)	// moving inwards
				{
					//2. See if new line degenerates a line segment - LEFT
					if (newLeftLeg.Cross(rightLeg) > 0.f)			// legs crossed
					{
						apexPos += rightLeg;
						apexIdx = rightLegIdx;
