...
void App_AgarioGame_BT::Start()
{
	...
	BehaviorTree* pBehaviorTree{ new BehaviorTree(pBlackboard, 
		...
		{
			// Flee enemy if a big one is close
			new BehaviorSequence(
			{
				new BehaviorConditional(BT_Conditions::IsBigAgentNearby),
				new BehaviorAction(BT_Actions::ChangeToFleeEnemy)
			}),

			// Chase enemy if a small one is close
			new BehaviorSequence(
			{
				new BehaviorConditional(BT_Conditions::IsSmallAgentNearby),
				new BehaviorAction(BT_Actions::ChangeToSeekEnemy)
			}),
...
Blackboard* App_AgarioGame_BT::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("SearchRadius", m_SearchRadius);
...
#pragma region UI
	...
	if (m_GameWon)
	{
		...
		ImGui::SetNextWindowPos(ImVec2(width / 2.0f - menuWidth, height / 2.0f - menuHeight));
		ImGui::SetNextWindowSize(ImVec2((float)menuWidth, (float)menuHeight));
		ImGui::Begin("You won", &windowActive, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
		ImGui::Text("Final Agent Info");
		ImGui::Text("Radius: %.1f", m_pSmartAgent->GetRadius());
		ImGui::Text("Time: %.1f", TIMER->GetTotal());
		ImGui::End();
	}
#pragma endregion
#endif
