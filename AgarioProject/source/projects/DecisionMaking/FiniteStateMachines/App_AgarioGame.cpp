...
void App_AgarioGame::Start()
{
	...
	FleeAgentState* pFleeAgent = new FleeAgentState();
	m_pStates.push_back(pFleeAgent);

	PursueAgentState* pPursueAgent = new PursueAgentState();
	m_pStates.push_back(pPursueAgent);

	//3. Create the conditions to transition between those states
	FoodNearByCondition* pFoodNearBy = new FoodNearByCondition();
	TargetFoodGoneCondition* pFoodEaten = new TargetFoodGoneCondition();
	BigAgentNearCondition* pBigAgent = new BigAgentNearCondition();
	BigAgentFarCondition* pNoBigAgent = new BigAgentFarCondition();
	SmallAgentNearCondition* pSmallAgent = new SmallAgentNearCondition();
	SmallAgentFarCondition* pNoSmallAgent = new SmallAgentFarCondition();

	m_pConditions.push_back(pFoodNearBy);
	m_pConditions.push_back(pFoodEaten);
	m_pConditions.push_back(pBigAgent);
	m_pConditions.push_back(pNoBigAgent);
	m_pConditions.push_back(pSmallAgent);
	m_pConditions.push_back(pNoSmallAgent);
	...
	// --> flee
	pStateMachine->AddTransition(pWanderState, pFleeAgent, pBigAgent);
	pStateMachine->AddTransition(pSeekFood, pFleeAgent, pBigAgent);
	pStateMachine->AddTransition(pPursueAgent, pFleeAgent, pBigAgent);

	// --> pursue
	pStateMachine->AddTransition(pWanderState, pPursueAgent, pSmallAgent);
	pStateMachine->AddTransition(pSeekFood, pPursueAgent, pSmallAgent);

	// --> seek food
	pStateMachine->AddTransition(pWanderState, pSeekFood, pFoodNearBy);

	// --> wander
	pStateMachine->AddTransition(pSeekFood, pWanderState, pFoodEaten);
	pStateMachine->AddTransition(pFleeAgent, pWanderState, pNoBigAgent);
	pStateMachine->AddTransition(pPursueAgent, pWanderState, pNoSmallAgent);
...
Blackboard* App_AgarioGame::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("FoodVec", &m_pFoodVec);	// reference to avoid copy
	pBlackboard->AddData("NearestFood", static_cast<AgarioFood*>(nullptr));
	pBlackboard->AddData("SearchRadius", 7.f);
	pBlackboard->AddData("AllAgents", &m_pAgentVec);
	pBlackboard->AddData("NearestAgent", static_cast<AgarioAgent*>(nullptr));
	//...
