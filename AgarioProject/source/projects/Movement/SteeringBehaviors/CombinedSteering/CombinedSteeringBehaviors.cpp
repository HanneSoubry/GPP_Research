...
SteeringOutput BlendedSteering::CalculateSteering(float deltaT, SteeringAgent* pAgent)
{
	...
	if (pAgent->CanRenderBehavior())
	{
		...
		//DEBUGRENDERER2D->DrawDirection(pAgent->GetPosition(), m_WeightedBehaviors[0].pBehavior->CalculateSteering(deltaT, pAgent).LinearVelocity, 7, {1, 0, 0}, 0.40f);
		//DEBUGRENDERER2D->DrawDirection(pAgent->GetPosition(), m_WeightedBehaviors[1].pBehavior->CalculateSteering(deltaT, pAgent).LinearVelocity, 7, {0, 1, 0}, 0.40f);
	}
